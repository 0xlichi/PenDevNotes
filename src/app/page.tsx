        ...
        <div>
          <div className="mb-8 animate-fade-in-up">
            ...
            <p className="mt-2 max-w-2xl text-[#7a6b56]">
              {notes.length} note{notes.length !== 1 ? 's' : ''} across {categories.length} categor
              {categories.length !== 1 ? 'ies' : 'y'}. Press{' '}
  ...
  return (
    <div className="animate-fade-in-up rounded-xl border border-dashed border-border bg-surface p-10 text-center">
      ...
      <p className="mt-1 text-sm text-[#7a6b56]">
        Add a <code className="rounded bg-[#f1e6d3] px-1.5 py-0.5 font-mono">.md</code> file to the{' '}
        <code className="rounded bg-[#f1e6d3] px-1.5 py-0.5 font-mono">/content</code> folder to see
        it appear here.
      </p>
    </div>
  );
