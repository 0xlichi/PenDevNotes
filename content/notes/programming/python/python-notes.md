...
- Download Python from [python.org](https://python.org/) - installs the Python **virtual machine (interpreter)** that converts `.py` code Üí bytecode Üí runs on the PVM.
...
```text
name  ÄÄÄÄñ∫  "Lichi"   (a string object in memory)
age   ÄÄÄÄñ∫  20        (an integer object in memory)
```
...
1. `age >= 18` Üí `True`
2. `has_ticket` Üí `True`
3. `True and True` Üí `True`
4. `True or is_vip (False)` Üí `True` (short-circuits: never even needs to check `is_vip`)
...
### Mini Challenge
 ...
 Write a recursive function `flatten(nested: list) -> list` that takes a list which might contain other lists inside it (nested to any depth) and returns one flat list of all the values. Example: `flatten([1, [2, 3, [4, 5]], 6])` Üí `[1, 2, 3, 4, 5, 6]`.
...
```text
original ÄÄñ∫ [ptr, ptr]
                Ç    Ç
                ñº    ñº
shallow ÄÄÄÄÄñ∫ [ptr, ptr]   (same inner list objects!)

deep ÄÄÄÄÄÄÄÄñ∫ [new_list, new_list]   (fully independent)
```
...
Text you see is Unicode characters; text stored/transmitted (files, network, HTTP) is **bytes**. Converting between them is **encoding** (str Üí bytes) and **decoding** (bytes Üí str).
...
# non-ASCII example
text = "caf√©"
print(text.encode("utf-8"))    # b'caf\xc3\xa9' : √© takes 2 bytes in UTF-8
```
...
BaseException
ÄÄ SystemExit
ÄÄ KeyboardInterrupt
ÄÄ Exception
    ÄÄ ValueError
    ÄÄ TypeError
    ÄÄ KeyError
    ÄÄ FileNotFoundError
    ÄÄ ... (your custom exceptions typically go here)
```
...
match value:
    case 1:
        ...
    # no case _: Üí if value is 2, NOTHING happens, no error, easy to miss
...
generator: (Ä)  produces 1, then 2, then 3, ... one at a time, on request
```
...
This is a **decorator factory**: `repeat(times=3)` is called first, and _its return value_ (`decorator`) is what actually gets applied to `greet`. Three layers: `repeat` (takes decorator arguments) Üí `decorator` (takes the function) Üí `wrapper` (takes the function's actual call arguments).
...
- Use named groups for anything with more than 1Äì2 capture groups.
...
- Test behavior (inputs Üí outputs), not internal implementation details: so refactors don't break tests unnecessarily.
...
3. Confirm: `print(data["Guest"])` Üí `[]`. Hypothesis confirmed.
...
An **IP address** identifies a machine on a network (e.g., `192.168.1.5`, or `127.0.0.1` for "this same machine": called **localhost**). A **port** identifies a specific _service_ running on that machine (a number from 0Äì65535). Together, `IP:port` (e.g., `192.168.1.5:443`) uniquely identifies one service on one machine.
...
call_api_1()  Äwait 2sÄñ∫  call_api_2()  Äwait 2sÄñ∫  call_api_3()  Äwait 2sÄñ∫  done
Total: ~6 seconds, one at a time

ASYNCHRONOUS (concurrent):
call_api_1()  ê
call_api_2()  Ä all waiting on network AT THE SAME TIME Äñ∫ done
call_api_3()  ò
...
- Many concurrent network calls / I/O waits Üí **asyncio** (most efficient for large numbers of concurrent I/O operations)
- A moderate number of blocking I/O calls, working with libraries that aren't async-aware Üí **threading**
- Heavy CPU computation that needs to use multiple cores Üí **multiprocessing**
- One simple, sequential task Üí plain **sync** code: don't add async complexity you don't need
...
### Mini Challenge
 ...
 Build an async "host checker" that takes a list of 10 fake hosts (each simulated with a random `asyncio.sleep` between 0.1Äì2 seconds, and a random chance of "failing"), runs all checks concurrently with a `TaskGroup`, and prints a final summary of which succeeded/failed and the total elapsed time: compare the elapsed time to what a fully sequential version would have taken.
...
CPU-bound (heavy computation, no waiting)   Üí multiprocessing
I/O-bound, few operations, blocking libs    Üí threading
I/O-bound, MANY concurrent operations       Üí asyncio
File I/O (usually fast, local)                Üí often fine sync, or threading if bulk
Network tasks (many requests)                   Üí asyncio (most efficient) or threading
```
...
- CPU-bound Üí `multiprocessing`. I/O-bound with many operations Üí `asyncio`. I/O-bound with a moderate number of operations, or working with non-async libraries Üí `threading`.
...
ÄÄÄÄÄÄÄÄÄÄÄÄÄê
Ç frame: b()  Ç  locals: {y: 2}
ÄÄÄÄÄÄÄÄÄÄÄÄÄ§
Ç frame: a()  Ç  locals: {x: 1}
ÄÄÄÄÄÄÄÄÄÄÄÄÄ§
Ç frame: <module>Ç
ÄÄÄÄÄÄÄÄÄÄÄÄÄò
```
