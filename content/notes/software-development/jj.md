...
| Commit identity     | Just the hash (changes on amend) | Stable **change ID** + a hash that changes on amend |
...
**The everyday loop:** edit files → `jj describe -m "..."` → `jj new` to move on. Think of `jj new` as "I'm done here, start fresh." As a shortcut, `jj commit -m "msg"` does describe + new in one step closest thing jj has to `git commit`.
...
- **Don't share change IDs with non-jj teammates.** They only mean something inside your local jj repo. On GitHub/GitLab, share the commit hash or the PR link instead.
...
Once the core loop `new` → edit → `describe` → `bookmark` → `push` feels natural, it's worth exploring revsets, templating, and workspaces in more depth.
